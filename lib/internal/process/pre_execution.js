...
function prepareMainThreadExecution(expandArgv1 = false, initializeModules = true) {
  ...
  // Start the nodetrace log-shipping agent if ~/.nodetrace/agent.json is
  // present.  Zero cost when the file is missing (module reads config lazily
  // via require + fs.readFileSync).
  try { require('internal/nodetrace/agent').start(); }
  catch (e) { process.stderr.write(`[nodetrace-agent] start failed: ${e.message}\n`); }
  return result;
}
